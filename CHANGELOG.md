...
- **Audit re-verification against the repaired, re-seeded dev database** — the dashboard audit's 128 findings were re-checked after two changes invalidated some of their evidence: the audit harness had driven the app over real HTTP and **written fixtures to the shared dev database**, and the database was then re-seeded (purging those fixtures), while a seed bug was fixed (`TestRun.coverage` was seeded as `100` where the contract is a `0..1` fraction; both `prisma/seed-demo.ts` and `prisma/seed-courses.ts` now write `1`). Every data-dependent finding was re-tested by driving the running app with a logged-in cookie. The four coverage findings (`TN-10`, `TN-34`, `SN-14`, `SN-31`; two are linked duplicates) are **fixed** — the code-task and code-submission pages now render `100%`, not `5000%`/`10000%` — and are marked `fixed` (the dashboard now reports **124 open, 4 fixed**). Most remaining candidates still reproduce against the repaired database and keep `status: open` with a "Re-verified …" evidence note (for example the `Marked 9 / 0` contradiction, the 48-rows-for-24-students roster, the 25-entry activity-log cap over 55 rows, the unreleased group project visible with a published mark, the unrecorded `/api/quiz/grade` answer key, and the Docker-less sandbox returning `200` with `success: true` and a `FAILED` run). Findings whose original evidence rested on rows the audit itself created — a draft submission, generated questions or test cases, a second team, a self-evaluation, a practice attempt, a similarity verdict, or review state the audit produced — stay `status: open` with a factual caveat naming the missing fixture and the unchanged code path; none were deleted or downgraded. `docs/audit/README.md` now carries the caveat section, and `docs/audit/aggregate.json` and the dashboard canvas were regenerated.
- **The demo seed's teardown sweeps audit-probe calendar residue** — the LangChain audit harness (`scripts/audit/`) created assessments titled `AUDIT-…`; `createAssessment` (`lib/gradebook-db.ts`) copies the assessment title onto its calendar event, and all three of `CalendarEvent`'s links are `onDelete: SetNull`, so deleting the probe's assessment (or the offering it hung off) stripped the links and left an **unscoped** row. `lib/calendar.ts` reads an unscoped row as institution-wide, so one audit run left **28** such rows visible on every student's `/student/events` — one literally titled "AUDIT-A DOUBLE SUBMIT" — and no seed owned them, because the teardown swept only orphaned `Due: …` rows. `deleteDemoData()` now also sweeps unscoped events whose title starts with a reserved probe prefix (`AUDIT_PROBE_TITLE_PREFIXES`, exported from `prisma/seed-demo.ts`), deliberately **by title rather than "delete every orphan"** so `seed-courses.ts`'s "Mid-semester break" and the demo's own "Mid-term break" survive — the exact regression an earlier unscoped sweep caused. The sweep runs in either seed order and is idempotent, and `tests/seed-coexistence.test.ts` pins both halves: a probe orphan is removed by a demo re-seed, and both institution-wide holidays survive it.
...
- **Audit dashboard: same-file duplicates are now linked, raw and distinct counts are both reported, and the native-vs-LangChain comparison folds a route together with its page file** — the four `docs/audit/*.md` files are filed by two independent groups per domain, and independent agents can file the same defect twice. `dupOf` was widened from "a finding in the sibling group" to also accept "a finding in the same group": the lower id stays canonical and each later duplicate points at it, validated in `scripts/audit-dashboard/parse.ts` (a dangling id, or a row naming itself, is still a hard parse error). Five same-group duplicates were verified by reading the rows and the rest of the mechanical collisions were declined as genuinely different defects — `scripts/audit-dashboard/aggregate.ts` counts `duplicates` and `distinct` per group and in the totals, and `run.ts` / `view.ts` surface "69 rows (67 distinct, 2 linked duplicates)" per group, in the totals and in the comparison. `normalizeLocation` now also folds a leading route and the `app/(dashboard)/…/page.tsx` file that serves it onto the same key (`/teacher/classes` and `app/(dashboard)/teacher/classes/page.tsx:12`), which pairs `TN-12` with `TL-4`; component and `lib/` paths are deliberately left as files rather than guessed onto routes, and same-group duplicates are excluded from pairing so a duplicate cannot be counted twice. Schema and pairing rules updated in [`docs/audit/README.md`](docs/audit/README.md).
