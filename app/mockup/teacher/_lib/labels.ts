...
/**
 * Domain → status vocabulary, shared by the teacher mockup pages.
 *
 * Mapping an enum to a `StatusKey` is a page concern (see
 * `docs/ui/design-system.md` §5), so it lives here rather than in the
 * primitive. Nothing in this module fetches or reads the clock; it is pure
 * lookup data plus two formatters.
 */
