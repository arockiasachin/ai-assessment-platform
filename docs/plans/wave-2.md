...
| Rule                          | Value                                                                                                                                                                                                                                       |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Continuous assessment / final | **CAM 60% + FAT 40%** for theory (CAT-I 15 + CAT-II 15 + 30 internal = 60). Lab and soft-skills are also 60/40. **Project courses have no FAT at all** (internal reviews only). The 30/70 figure is VIT _Online_ — a different directorate. |
...
| Pass boundary                 | **`min(mean − 2σ, 50)`** — capped at 50, never floored to it. Below 50 the bar drops to `mean − 2σ`; above 50 nobody at or above 50 fails. (This entry was inverted; see `wave-3.md` §7.6.)                                                 |
...
| **When relative applies**     | theory and lab-embedded theory with class strength **> 10**; a class of **≤ 10** is graded **absolutely** instead                                                                                                                           |
| **Always absolute**           | **laboratory, soft-skills, extra-curricular, NGCR and project** courses, "irrespective of the class strength"                                                                                                                               |
| Absolute band widths          | `S ≥ 90`, `A 80–90`, `B 70–80`, `C 60–70`, **`D 55–60`, `E 50–55`**, `F < 50`                                                                                                                                                               |
| Non-band pass gates           | theory needs ≥ 40% in the FAT alone; lab/project need ≥ 50% of their component; grand total ≥ 50% for every type except theory-only                                                                                                         |
| `S` cap triggers              | **two**: the course has no `S` grade, **or** `mean + 1.5σ > 100`. Award goes to the top `max(3, 5%)` of the class (v5.0 adds `≥ 80` to that cap case)                                                                                       |
