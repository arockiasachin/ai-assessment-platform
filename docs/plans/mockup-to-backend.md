...
Status: **complete — Waves 0–4 landed.** Written against `dev` @ `2bc51b5` (after the UI mockup
work merged and CI passed). The remaining open items are the decisions recorded in
[`wave-4.md`](./wave-4.md) §6–§9, not unfinished slices.
...
| ~~**Similarity unique constraint**~~          | ~~`SimilarityCheck`'s dedupe key has two nullable columns; in Postgres NULL makes unique rows non-conflicting, so duplicate pairs can insert.~~                                                                                        | **Confirmed and fixed.** Two identical rows for one pair were reproduced against a real database, so this was a live defect rather than a theoretical one. `codeTaskId` is now required and the key is `[codeTaskId, studentId, comparedStudentId]`, leaving no nullable column in it. See [`wave-4.md`](./wave-4.md) §9. |
| ~~**Missing index**~~                         | ~~`Assessment` is filtered by `createdById` (`lib/gradebook-db.ts:170`) with no index on that column.~~                                                                                                                                | **Added** in `20260917030000_assessment_creator_index`, once the port touched that path. See [`wave-4.md`](./wave-4.md) §9.                                                                                                                                                                                               |
